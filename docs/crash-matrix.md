...
| Foreground notification | Delivery-intent commit, queue spawn, command outcome, and durable observation | An independent queue ledger records at most one attempt. Repeated recovery preserves uncertain delivery state, never retransmits the notice, and leaves inbox acknowledgement explicit. |
